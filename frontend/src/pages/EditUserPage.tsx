...
export async function editUserAction({ request }: ActionFunctionArgs) {
    ...
    const user = await fetch("/api/user", {
        ...
        if (response.ok) {
            ...
            errors.push(EditUserErrors.USERNAME_TAKEN);
        } else if (response.status === 409) {
            errors.push(EditUserErrors.INVALID_OLD_PASSWORD);
        }
            ...
            <Container>
                <Row>
                    <Col className="text-center">
                        {user && (
                            <Image
                                src={"https://robohash.org/" + user.username + ".png?set=set5&size=360x360"}
                                className="mt-2"
                                fluid
                                roundedCircle
                            />
                    ...
                    <Col>
                        <Form method="post">
                            ...
                            <FormGroup className="mb-3 mt-3" controlId="formEditUsername">
                                ...
                                />
                                {errors?.includes(EditUserErrors.USERNAME_TAKEN) && (
                                    <FormText className="text-danger-emphasis">{EditUserErrors.USERNAME_TAKEN}</FormText>
                                )}
                                </FormGroup>
                            <FormGroup className="mb-3" controlId="formOldPassword">
                                ...
                                {newAndOldPasswordsAreTheSame && (
                                    <FormText className="text-danger-emphasis">New and old passwords are the same</FormText>
                                )}
                                {errors?.includes(EditUserErrors.INVALID_OLD_PASSWORD) && (
                                    <FormText className="text-danger-emphasis">{EditUserErrors.INVALID_OLD_PASSWORD}</FormText>
                                )}
                            </FormGroup>
                            ...
                            {csrfToken && (
                                <FormGroup className="mb-3" controlId="formEditCsrfToken">
